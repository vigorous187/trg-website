...
| Release safety | Cloudflare credential separation | PASS | Manual release | Dedicated `CLOUDFLARE_ZARAZ_READ_TOKEN` is confined to exact Zaraz GET; Pages token stays confined to Pages steps | Workflow and request-builder contract tests; `docs/cloudflare-release-credentials.md` | GitHub secret must be provisioned before release |
